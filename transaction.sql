...
-- autocommit 을 비활성화 하면 transction 을 명시적으로 commit
-- 해야지만 변경한 사항이 데이터베이스에 영구적으로 반영이 된다.alter
-- start transaction 구문을 작성하고
-- dml (insert, delet, update) 등 작업 수행 후 commit or rollback 을 하면 된다.alter
