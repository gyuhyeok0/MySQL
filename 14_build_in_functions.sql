...
select concat_ws ( '-', '2023', '03', '04');

-- elt : 해당 위치의 문자열 반환
-- field : 찾을 문자열 위치 반환
-- find_in_set : 찾을 문자열의 시작 위치 반환
...
select format(123123123123.123124123, 3);
...
-- upper, lower 대문자 소문자 변경alter

select upper('Hello World!'), lower('Hello World');

-- 1pad( 문자열, 길이, 채울 문자열), rpad( 문자열, 길이, 채울 문자열)
-- 문자열을 길이 만큼 왼쪽으로 늘린 후에 빈 곳을 문자열로 채운다.
-- 0013130-1***** 

select 1pad('왼쪽', 6, '@'), rpad('오른쪽', 6, '@');

-- ltrim(문자열), rtrim(문자열)
-- 왼쪽공백 제거, 오른쪽 공백제거alter
select ltrim( '    왼쪽');

-- trim(문자열), trim( 방향 자를문자열 from 문자열)
-- 방향 : leading (앞), both(양쪽), trailing(뒤) 해당 방향에 대한 지정한 문자열을 제거할 수 있다.
select trim(both '@' from '@@@@MySQL@@@2'); 
...
-- 구분자가 왼쪽부터 횟수 번째 나오면 기 이후의 오른쪽은 버린다.alter
...
-- floor( (rand() * (n - m) + 1)
-- 1 부터 10 까지의 난수 : floor(rand() * (11-1) +1)

select rand(), floor(rand() * (11-1) +1);

-- sign(숫자) 
-- 양수면 1, 0 이면 0, 음수면 -1 을 반환

select sign(10.1), sing(0), sign(-10);

-- truncate(숫자, 정수);
-- 소수점을 기준으로 정수 위까지 구하고 나머지 버림alter
select truncate (12345.1234, -3);

-- 날짜 및 시간 관련 함수
-- adddate( 날짜, 차이) : 날짜를 기준으로 차이를 더함
-- subdate( 날짜, 차이) : 날짜를 기준으로 날짜를 뺌
select adddate( ' 2023-03-04', interval 30 day);

select addtime( ' 2023-03-04 17:42:00 ', '00:00:30');
select subtime( ' 2023-03-04 17:42:00 ', '00:00:30');
...
-- 연 월 일 시 분 초 밀리초
select year( curdate()), month(curtime());


-- datediff( 날짜 1, 날짜 2)
-- timediff( 날짜 1 or 시간 1, 날짜 2 또는 시간 2)
-- dateoff : 날짜 1 - 날짜 2의 일수를 반환
-- timediff : 시간 1 - 시간 2의 결과alter

select datediff('2024-03-04', now()), timediff('17:05:11' , '13:00:10');
