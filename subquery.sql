...
-- tbl_menu 테이블에서 미트미역국과 같은 카테고리 코드를 가지고 있는 행들의 모든
-- 정보를 조회하시오.
...
-- from 절에 쓰인 서브쿼리(derived tabel, 파생 테이블)
-- 반드시 자신의 별칭이 있어야 한다.
-- 이 count 라는 컬럼이 가진 가장 큰 값 -> 6 이라는 값을 도출하고 싶다.

-- max() 최대값을 반환해줌.

select max(count) from ( select category_code, count(*) as 'count' from tbl_menu group by category_code) as countmenu;
...
-- 카테고리별 평균 가격보다 높은 가격의 메뉴의 메뉴코드, 메뉴이름, 카테고리코드 를 조회하시오.

select a.menu_code, a.menu_name, a.category_code from tbl_menu a where menu_price > (select avg(menu_price) from tbl_menu where category_code = a.category_code);
-- where category_code = a.category_code) 이거 쓰는이유 모름

-- cte(common table expressions)
-- 패생 테이블과 비슷한 개념이며 코드의 가독성과 재사용성을 위해 파생 테이블 대신 사용하게 된다.
