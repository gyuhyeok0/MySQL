...
-- 1-1 create
-- 테이블 생성을 위한 구문
-- if not exists 를 적용하면 기존에 존재하는 테이블이라도 에러가 발생하지 않는다.alter
...
create table if not exists tb1 (
	pk int primary key, -- pk 라는 컬럼은 식별자 
    fk int,
    col1 varchar(255),
...
insert into tb1 values(2, 10, 'Y');

select * from tb1;

-- auto_increment
-- insert 시 primary 키에 해당하는 컬럼에 자동으로 번호를 발생(중복되지 않게) 시켜 저장 할 수 있다.
...
-- 열 추가
-- alter table 테이블명 add 컬럼명 컬럼정의alter
...
-- fk -> change_fx 컬럼으로 변경 not null 제약조건 추가

alter table tb2
change column fk change_ftb1tb1fkk int not null;
...
alter table tb2
modify pk int;       -- modify 명령어로 auto_increment 제거

describe tb2;

alter table tb2       -- primary Key 제거
drop primary key;
...
-- date 형이 추가가 안되는 것은 MySql 이 5.7 버전 이후 
-- 0 으로 채워진 date 컬럼이 존재하면 안 되기 때문이다.alter

select @@global.sql_mode;      -- << 전역변수 느낌 ,, 루트에 set global sql_mode = '';
