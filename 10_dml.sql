...
select * from tbl_menu; -- 값 조회용

-- 값을 넣을거야 어디에? tbl_menu 그러면 값은 ?  values ();
insert into tbl_menu values (null, '바나나해장국', 8500, 5, 'Y' );

-- 테이블 정보 확인
describe tbl_menu;

-- auto increment : 자동으로 증가되는 기능 / 값을 넣지 않는다면 자동으로 번호를 부여받
...
-- replace : 특정 행이 이미 존재하는 경우에는 행을 갱신
-- 그렇지 않은 경우 새 행을 삽입
-- 즉, 해당 행을 삭제하고 새로운 값을 삽입하는 것이다.

select * from tbl_menu; -- 조회용
